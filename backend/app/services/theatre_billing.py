"""Theatre charge helper — mirrors dialysis_billing / maternity_billing.

Finds-or-creates the patient's Pending invoice under FOR UPDATE, appends a
Theatre line item, bumps the total, and GL-posts via post_from_event.
Zero-priced / missing / inactive service codes charge nothing (returns None).
The CALLER owns the commit — the charge lives or dies with the case row.
"""
...
def raise_theatre_charge(
    ...
    *,
    ...
    log_audit(db, user_id, "CREATE", "InvoiceItem", item.id, None,
              {"amount": float(amt), "type": "Theatre", "service_code": service_code}, None)
    return item
