...
def _settle_subscription_invoice(master_db: Session, *, txn: PlatformMpesaTransaction) -> Optional[str]:
    ...
    Returns None when the ledger was updated, or already carried this exact
    receipt from an earlier delivery (a true no-op, not a failure). Returns
    a human-readable reason, instead of applying anything, when the
    settlement cannot be safely credited: the caller then quarantines the
    whole transaction rather than treating the row as Success.

    CRITICAL fix, round 1 review: this used to have NO balance check at
    all, unlike its own sibling app/routes/receivables.py's record_payment
    (lines 264-269), which rejects a manual overpayment outright with a
    400. That option does not exist here: the money already moved on
    Safaricom's side by the time a callback reaches this function, so
    refusing to record it anywhere would be its own, silent loss. Two
    genuine STK approvals against the same invoice (a superadmin
    double-click; the partial unique index in
    app/services/daraja/platform_stk.py's _reserve_pending stops a SECOND
    prompt from ever being sent, but does nothing for two prompts that
    were already both approved before that index existed, or any other
    way two Pending rows end up pointing at one invoice) produce two
    different Safaricom receipts, so the receipt-keyed replay check in
    apply_platform_stk_callback never fires for either: both reach here.
    Without this check, the second one drove amount_kes total above
    amount_kes on the invoice, outstanding_balance went NEGATIVE, status
    stayed "paid", and nothing anywhere recorded that anything was wrong.
    Ruling: quarantine an over-the-balance settlement rather than silently
    reject it. The visibility this buys is only as good as
    _notify_quarantine's ERROR log plus the transactions list route until
    the quarantined count is surfaced on the operator health panel
    (tracked for the frontend task).

    A no-op (returns None) when the push was never tied to an invoice
    ...
    if invoice is None:
        return (
            f"subscription_invoice_id={txn.subscription_invoice_id} no longer "
            "exists; receipt received but not credited to any invoice"
        )
    if invoice.status == "void":
        return (
            f"subscription invoice {invoice.id} is void; receipt received "
            "but not credited"
        )
    ...
    if existing is not None:
        return None  # already applied by an earlier delivery of this receipt

    amount = Decimal(str(txn.amount or 0))
    balance = outstanding_balance(master_db, invoice)
    if amount > balance:
        return (
            f"settlement of KES {amount} exceeds outstanding balance KES "
            f"{balance} on invoice {invoice.number}; money received but "
            "NOT credited, needs manual review"
        )
...
def apply_platform_stk_callback(
    ...
    try:
        ...
        # A non-None reason means the money arrived but could NOT be
        # safely credited (see _settle_subscription_invoice's docstring,
        # CRITICAL fix round 1): quarantine instead of marking Success, so
        # a genuinely overpaid or orphaned receipt is never recorded as a
        # clean settlement.
        quarantine_reason = _settle_subscription_invoice(master_db, txn=txn)
        if quarantine_reason:
            txn.status = "Quarantined"
            txn.result_desc = quarantine_reason[:255]
            master_db.commit()
            _notify_quarantine(txn, reason=txn.result_desc)
            return _snapshot(txn)
