...
def register_patient(patient_in: PatientCreate, request: Request, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    # Audit PER-001: previously this took `patient_data: dict` and mass-assigned
    # every supplied key onto the Patient ORM model. PatientCreate now locks the
    # write surface — unknown fields raise 422 at the boundary.
    try:
