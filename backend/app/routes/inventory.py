...
@router.post("/items", response_model=InventoryItemResponse, dependencies=[Depends(RequirePermission("inventory:manage"))])
def create_item(item_in: InventoryItemCreate, request: Request, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    ...
    log_audit(db, current_user["user_id"], "CREATE", "InventoryItem", new_item.item_id, None, item_in.model_dump(), request.client.host)
    db.commit()
    db.refresh(new_item)
    return new_item

@router.post("/batches", response_model=StockBatchResponse, dependencies=[Depends(RequirePermission("inventory:manage"))])
def add_stock_batch(batch_in: StockBatchCreate, request: Request, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    ...
    log_audit(db, current_user["user_id"], "CREATE", "StockBatch", new_batch.batch_id, None, batch_in.model_dump(), request.client.host)
    db.commit()
    db.refresh(new_batch)
    return new_batch

@router.post("/transfer", dependencies=[Depends(RequirePermission("inventory:manage"))])
...
@router.get("/alerts", dependencies=[Depends(RequirePermission("inventory:manage"))])
