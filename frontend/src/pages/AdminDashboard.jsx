        ...
        <div className="flex flex-col gap-4 h-full md:h-[calc(100vh-8rem)] min-h-[calc(100vh-8rem)]">
                        ...
                        <table className="table-clean table-sticky min-w-[800px]">
                            <thead>
                                <tr>
                                    ...
                                    <th>License / Specs</th>
                                    <th>Status</th>
                                    <th className="num">Security & RBAC</th>
                                </tr>
                            </thead>
                            <tbody>
                                {isLoading ? (
                                    <tr><td colSpan="5"><SkeletonTable rows={5} cols={5} label="Loading directory" /></td></tr>
                                    ...
                                    filteredStaff.map((user) => {
                                        ...
                                        return (
                                            <tr key={user.user_id} className="hover:bg-slate-50 transition-colors">
                                                ...
                                                <td className="num flex justify-end gap-2">
                                                    <button type="button"
                                ...
                                {isLoading ? (
                                    ...
                                    filteredPricing.map((item) => (
                                        <tr key={item.catalog_id} className="hover:bg-slate-50 transition-colors">
                                            <td className="font-bold text-slate-900">{item.test_name}</td>
                                            <td>
                        ...
                        <table className="table-clean table-sticky min-w-[800px]">
                            <thead>
                                <tr>
                                    ...
                                    <th>Operation</th>
                                    <th>Entity & ID</th>
                                    <th className="w-1/3">Data Snapshot</th>
                                </tr>
                            </thead>
                            <tbody>
                                {isLoading ? (
                                    ...
                                    auditLogs.map((log) => (
                                        <tr key={log.log_id} className="hover:bg-slate-50">
                                            <td className="text-slate-500">{new Date(log.timestamp).toLocaleString()}</td>
                                            <td className="font-bold text-slate-800">{log.user_id || 'SYSTEM'}</td>
                                            <td>
                                            ...
                                            <td className="text-brand-700 font-bold">{log.entity_type} [{log.entity_id}]</td>
                                            <td className="text-slate-400 truncate max-w-xs">{JSON.stringify(log.new_value || log.old_value)}</td>
                                        </tr>
                                    ))
                                )}
                            </tbody>
            ...
            {isPricingModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center">
                    ...
                    <div className="relative w-full max-w-md bg-white dark:bg-ink-900 rounded-xl shadow-2xl flex flex-col overflow-hidden animate-slide-up">
            ...
            {isEditRoleModalOpen && selectedUser && (
                <div className="fixed inset-0 z-50 flex items-center justify-center">
                    ...
                    <div className="relative w-full max-w-sm bg-white dark:bg-ink-900 rounded-xl shadow-2xl flex flex-col overflow-hidden animate-slide-up">
                        <div className="p-6 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
                            <div>
