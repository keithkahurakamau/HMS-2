...
import { 
    ...
    AlertCircle, Search, UserPlus, Lock, Unlock, CheckCircle2,
    X, ShieldCheck, Database, FileDigit, Edit
} from 'lucide-react';
...
export default function AdminDashboard() {
    ...
    return (
        <div className="h-[calc(100vh-8rem)] flex flex-col gap-4">
            ...
            {/* HEADER & TABS */}
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-2 shadow-sm flex items-center justify-between shrink-0">
                <div className="flex bg-slate-800 p-1 rounded-lg w-full max-w-xl">
                    <button onClick={() => setActiveTab('overview')} className={`flex-1 flex items-center justify-center gap-2 py-2 px-4 rounded-md text-sm font-bold transition-all ${activeTab === 'overview' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-300 hover:text-white'}`}>
                        <LayoutDashboard size={18} /> System Overview
                    </button>
                    <button onClick={() => setActiveTab('staff')} className={`flex-1 flex items-center justify-center gap-2 py-2 px-4 rounded-md text-sm font-bold transition-all ${activeTab === 'staff' ? 'bg-brand-500 text-white shadow-sm' : 'text-slate-300 hover:text-white'}`}>
                        <Users size={18} /> Staff Directory
                    </button>
                    <button onClick={() => setActiveTab('audit')} className={`flex-1 flex items-center justify-center gap-2 py-2 px-4 rounded-md text-sm font-bold transition-all ${activeTab === 'audit' ? 'bg-red-500 text-white shadow-sm' : 'text-slate-300 hover:text-white'}`}>
                        <ShieldAlert size={18} /> Security Audit
                    </button>
                </div>
                <div className="text-right px-4 text-sm font-bold text-slate-400 flex items-center gap-2">
            ...
            {/* TAB 2: STAFF DIRECTORY & PROVISIONING */}
            {activeTab === 'staff' && (
                                        ...
                                        <tr key={user.user_id} className="hover:bg-slate-50 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-right flex justify-end gap-2">
                                                ...
                                                <button 
                                                    ...
                                                    className={`text-xs font-bold px-3 py-1.5 rounded border transition-colors ${user.is_active ? 'border-slate-200 text-slate-600 hover:bg-red-50 hover:text-red-600 hover:border-red-200' : 'bg-slate-800 text-white hover:bg-slate-900'}`}
                                                >
            ...
            {/* TAB 3: SYSTEM AUDIT LOGS (remains unchanged) */}
            {activeTab === 'audit' && (
                <div className="flex-1 bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden flex flex-col">
            ...
            {/* --- ADD STAFF MODAL --- (Remains unchanged) */}
            {isStaffModalOpen && (
