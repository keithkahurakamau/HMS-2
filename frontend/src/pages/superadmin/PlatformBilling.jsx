    ...
    return (
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="card overflow-hidden">
                <div className="p-4 border-b border-ink-200 dark:border-ink-800 flex flex-col sm:flex-row sm:flex-wrap sm:justify-between sm:items-center gap-3">
                    <h2 className="text-sm font-semibold text-ink-900 dark:text-white tracking-tight">Tenant subscriptions</h2>
                    <div className="relative w-full sm:w-auto">
                        ...
                        <input
                            ...
                            className="w-full sm:w-64 bg-white dark:bg-ink-900 border border-ink-200 dark:border-ink-800 rounded-lg pl-9 pr-3 py-1.5 text-sm text-ink-900 dark:text-white placeholder-ink-400 focus:outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 transition-all"
                        />
                    </div>
                </div>
                                    ...
                                    <td className="px-6 py-3">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div className="min-w-0">
                                                <p className="font-semibold text-ink-900 dark:text-white truncate">{tenant.name}</p>
                                                <p className="text-xs text-ink-500 dark:text-ink-400 font-mono truncate">{tenant.domain}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-3">
                ...
                <ul className="md:hidden divide-y divide-ink-100 dark:divide-ink-800">
                    {isLoading ? (
                        <li className="p-8 text-center text-ink-500 dark:text-ink-400">
                            <Activity size={16} className="inline animate-spin mr-2 text-brand-600 dark:text-brand-400" aria-hidden="true" />Loading…
                        </li>
                    ) : filtered.length === 0 ? (
                        <li className="p-8 text-center text-ink-500 dark:text-ink-400">No tenants match your filter.</li>
                    ) : filtered.map(tenant => (
                            ...
                            <div className="flex items-start justify-between gap-3">
                                <div className="min-w-0 flex-1">
                                    ...
                                    <div className="mt-2 flex flex-wrap items-center gap-2">
                                        ...
                                        <span className="text-xs font-mono text-ink-700 dark:text-ink-200">
            ...
            <p className="text-xs text-ink-500 dark:text-ink-400 flex items-start gap-2 px-2 leading-relaxed">
...
const KPI_ACCENT = {
    accent: 'bg-accent-50 ring-accent-100 text-accent-700 dark:bg-accent-500/10 dark:ring-accent-500/20 dark:text-accent-300',
    brand:  'bg-brand-50  ring-brand-100  text-brand-700  dark:bg-brand-500/10  dark:ring-brand-500/20  dark:text-brand-300',
    amber:  'bg-amber-50  ring-amber-100  text-amber-700  dark:bg-amber-500/10  dark:ring-amber-500/20  dark:text-amber-300',
    teal:   'bg-teal-50   ring-teal-100   text-teal-700   dark:bg-teal-500/10   dark:ring-teal-500/20   dark:text-teal-300',
};
    ...
    return (
        <div className={`rounded-2xl border p-5 ${
            ...
            <div className="flex justify-between items-start mb-3">
                <h3 className={`text-lg font-semibold tracking-tight ${highlight ? 'text-amber-900 dark:text-amber-200' : 'text-ink-900 dark:text-white'}`}>{tier}</h3>
                {highlight && <Crown size={18} className="text-amber-700 dark:text-amber-400" aria-hidden="true" />}
            </div>
            <div className="flex items-baseline gap-1 mb-1">
                <span className="text-2xl font-semibold tracking-tight text-ink-900 dark:text-white">{price}</span>
                <span className="text-sm font-medium text-ink-500 dark:text-ink-400">/mo</span>
            </div>
            <div className="text-xs text-ink-500 dark:text-ink-400 mb-4">{seats} active subscriber{seats !== 1 ? 's' : ''}</div>
            <ul className="space-y-1.5 text-xs text-ink-700 dark:text-ink-200">
                {features.map(f => (
                    <li key={f} className="flex items-start gap-2">
                        <Check size={14} className={`shrink-0 mt-0.5 ${highlight ? 'text-amber-700 dark:text-amber-400' : 'text-accent-600 dark:text-accent-400'}`} aria-hidden="true" />
                        <span>{f}</span>
                    </li>
                ))}
            </ul>
