...
function StatusBadge({ user }) {
    if (!user.is_active) {
        return <span className="badge bg-rose-50 text-rose-700 ring-1 ring-rose-200 inline-flex items-center gap-1"><UserX size={11} aria-hidden="true" /> Disabled</span>;
    }
    if (user.is_locked) {
        return <span className="badge bg-amber-50 text-amber-700 ring-1 ring-amber-200 inline-flex items-center gap-1"><Lock size={11} aria-hidden="true" /> Locked</span>;
    }
    if (user.must_change_password) {
        return <span className="badge bg-sky-50 text-sky-700 ring-1 ring-sky-200 inline-flex items-center gap-1"><KeyRound size={11} aria-hidden="true" /> Must reset</span>;
    }
...
export default function UsersManager() {
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="bg-sky-50 border border-sky-200 text-sky-900 rounded-xl p-3 text-xs flex items-start gap-2">
                <ShieldCheck size={14} className="text-sky-600 shrink-0 mt-0.5" aria-hidden="true" />
                <span>
            ...
            {isLoading ? (
                ...
                <div className="card p-12 text-center text-ink-500">No users match your filters.</div>
            ) : (
                        ...
                        return (
                            <div key={key} className="card overflow-hidden">
                                <div className="px-5 py-3 border-b border-ink-200 bg-ink-50 flex items-center gap-2">
                                    ...
                                    <span className="text-2xs text-ink-500 font-mono ml-auto shrink-0">{rows.length} user{rows.length === 1 ? '' : 's'}</span>
                                </div>
                                    ...
                                    <table className="w-full text-left text-sm">
                                        ...
                                        <tbody className="divide-y divide-ink-100 text-ink-700">
                                            {rows.map(u => {
                                                ...
                                                return (
                                                    <tr key={keyOf(u)} className="hover:bg-ink-50 transition-colors">
                                                        <td className="px-5 py-3 font-semibold text-ink-900">{u.full_name}</td>
                                                        <td className="px-5 py-3 text-xs text-ink-600">{u.email}</td>
                                            ...
                                            <li key={`${keyOf(u)}-mb`} className="p-4 space-y-3">
                                                <div className="flex items-start justify-between gap-3">
                                                    <div className="min-w-0">
                                                        ...
                                                        <p className="text-xs text-ink-500 mt-0.5">{u.role || '—'}</p>
                                                    </div>
                                                    <StatusBadge user={u} />
                                                </div>
                    ...
                    <div className="relative w-full max-w-md bg-white rounded-2xl shadow-elevated border border-ink-200 p-6 animate-slide-up">
                        <div className="flex items-start justify-between gap-3 mb-4">
                            ...
                            <button type="button" onClick={() => setTempResult(null)} aria-label="Close"
                                className="p-2 text-ink-500 hover:text-ink-900 hover:bg-ink-100 rounded-lg cursor-pointer">
                                <X size={18} aria-hidden="true" />
                            </button>
                        </div>
                        <p className="text-sm text-ink-600 mb-4">
                            Share this with <strong className="text-ink-900">{tempResult.user.full_name}</strong> ({tempResult.user.email}) over a secure channel.
                            It’s shown <strong>once</strong> and they must change it at next login.
                        </p>
                        <div className="flex items-center gap-2 bg-ink-50 border border-ink-200 rounded-xl p-3">
                            <code className="flex-1 font-mono text-sm text-ink-900 break-all select-all">{tempResult.temporary_password}</code>
                            <button type="button" onClick={copyTemp} aria-label="Copy temporary password"
                                className="shrink-0 p-2 rounded-lg text-ink-500 hover:text-brand-700 hover:bg-ink-100 transition-colors cursor-pointer">
                                {copied ? <Check size={16} className="text-accent-600" aria-hidden="true" /> : <Copy size={16} aria-hidden="true" />}
                            </button>
                        </div>
                        <div className="mt-4 bg-amber-50 border border-amber-200 text-amber-900 rounded-xl p-3 text-2xs flex items-start gap-2">
