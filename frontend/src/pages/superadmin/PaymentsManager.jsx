            ...
            {!selected ? (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2 bg-white dark:bg-ink-900 border border-ink-200/70 dark:border-ink-800 rounded-2xl shadow-soft p-6 space-y-5">
                        ...
                        <p className="text-xs text-ink-500 dark:text-ink-400 mt-2 mb-3">
                        ...
                        <p className="text-xs text-ink-500 dark:text-ink-400 -mt-2 mt-1">
                        ...
                        <p className="text-xs text-ink-500 dark:text-ink-400 -mt-2 mt-1 inline-flex items-start gap-1.5">
                    ...
                    <div className="space-y-4">
                        ...
                        <div data-tour="prov-test" className="bg-white dark:bg-ink-900 border border-ink-200/70 dark:border-ink-800 rounded-2xl shadow-soft p-5 space-y-3">
                            ...
                            {config?.configured && !config?.mpesa_active && (
                                <p className="text-xs text-amber-700 dark:text-amber-300">Save a channel id first to enable the test.</p>
                            )}
                            {config?.last_test_at && (
                                <div className="text-xs text-ink-600 dark:text-ink-400 pt-2 border-t border-ink-100 dark:border-ink-800">
                                    ...
                                    {config.last_test_message && (<div className="text-ink-500 dark:text-ink-400 mt-1">{config.last_test_message}</div>)}
                                </div>
                            )}
                        </div>
                    </div>
    ...
    return (
        <div className="bg-white dark:bg-ink-900 border border-ink-200/70 dark:border-ink-800 rounded-2xl p-5 text-sm space-y-2">
            {config.mpesa_active ? (
                ...
                <div className="flex items-center gap-2 font-semibold text-amber-700 dark:text-amber-300"><AlertCircle size={16} /> Channel not set</div>
            )}
            <div className="text-ink-700 dark:text-ink-200">
                {config.shortcode && <div><span className="text-ink-500 dark:text-ink-400">Shortcode:</span> <span className="font-mono">{config.shortcode}</span> ({config.shortcode_type})</div>}
                {config.payhero_channel_id && <div><span className="text-ink-500 dark:text-ink-400">Channel:</span> <span className="font-mono">{config.payhero_channel_id}</span></div>}
                {config.settlement_bank_name && <div><span className="text-ink-500 dark:text-ink-400">Settles to:</span> <span className="font-mono">{config.settlement_bank_name} · {config.settlement_account_number}</span></div>}
                <div className="text-xs inline-flex items-center gap-1.5 text-ink-500 dark:text-ink-400 pt-1">
                    <KeyRound size={12} /> {config.uses_per_tenant_creds ? 'Per-tenant credentials' : 'Platform default credentials'}
                </div>
                <div className="text-xs inline-flex items-center gap-1.5 text-ink-500 dark:text-ink-400">
